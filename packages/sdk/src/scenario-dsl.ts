...
/**
 * Scenario DSL (ROADMAP M2: the DSL lives in @harness/sdk so that task
 * manifests, scenarios, and run reports are validated by ONE
 * vocabulary; YAML text → typed invariants).
 *
 * A scenario is an eval test case: it names the task(s) it exercises
 * and the observable invariants a correct golden run must produce.
 *
...
/**
 * A single event invariant. `type` is required and must be a known
 * wire type from @harness/events (rule 4: unknown input is a typed
 * error, never a silent fallback). Any `data.<path>` key asserts a
 * nested value on the event payload, e.g. `data.status: completed` —
 * exactly the shape in evals/scenarios/README.md.
 */
...
/** Parse + validate YAML text (a scenario file) into a typed Scenario. */
